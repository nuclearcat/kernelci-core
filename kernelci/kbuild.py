...
"""
    ...
    def submit(self, retcode, dry_run=False):
        ...
        if 'kernel' not in af_uri and not self._dtbs_check \
           and job_result == 'pass' and self._arch != 'um':
            print("[_submit] Kernel image not found in artifacts, setting job_result to fail")
            self.submit_failure(
                "Kernel image not found in artifacts, setting job_result to fail", af_uri)
